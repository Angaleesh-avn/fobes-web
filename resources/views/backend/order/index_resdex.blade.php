                ...
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between">
                            <h3 class="card-title line-height-36">{{ __('orders') }} Resdex</h3>
                            <a href="{{ route('order.create_resdex') }}" class="btn bg-primary float-right d-flex align-items-center justify-content-center">
                                <i class="fas fa-plus"></i>
                                {{ __('create') }}
                            </a>
                            @if (request('company') || request('provider') || request('plan') || request('sort_by'))
                                <div>
                                    <a href="{{ route('order.index_resdex') }}" class="btn bg-danger"><i class="fas fa-times"></i>
                                        &nbsp;{{ __('clear') }}
                                    </a>
                                </div>
                            @endif
                    ...
                    <form id="filterForm" action="{{ route('order.index_resdex') }}" method="GET">
                            ...
                            <tbody>
                                @forelse ($orders as $order)
                                    <tr>
                                        ...
                                        <td>
                                            <!--{{ currencyPosition(currencyConversion($order->usd_amount, 'USD', config('templatecookie.currency', 'USD')),false, $current_currency) }}-->
                                            {{ currencyPosition($order->amount) }}
                                        </td>
                                        <td>
                                            ...
                                            @else
                                                ...
                                                <div>
                                                    <a onclick="return confirm('{{ __('are_you_sure') }}')"
                                                        href="{{ route('manual.payment.mark.paid_resdex', $order->id) }}">
                                                        {{ __('mark_as_paid') }}
                                                    </a>
                                                </div>
                                            @endif
                                        ...
                                        <td class="d-flex align-items-center">
                                            <a href="{{ route('order.show_resdex', $order->id) }}" class="btn ll-btn ll-border-none">
                                                {{ __('view_details') }}
                                                <x-svg.table-btn-arrow />
                                            </a>
                                            @if (userCan('order.download'))
                                                <form
                                                    action="{{ route('admin.transaction.invoice.download_resdex', $order->id) }}"
                                                    method="POST">
                                                    @csrf
                                            ...
                                            @endif
                                            @if (userCan('order.download'))
                                                <form action="{{ route('admin.transaction.invoice.view_resdex', $order->id) }}"
                                                    method="POST">
                                                    @csrf
