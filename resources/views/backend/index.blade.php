    ...
    <div class="row">
                            ...
                            @if ($latest_earnings->count() > 0)
                                @foreach ($latest_earnings as $earning)
                                    <tr>
                                        ...
                                        <td class="text-muted">
                                        <!--{{ currencyPosition(currencyConversion($earning->usd_amount, 'USD', config('templatecookie.currency', 'USD') ), false,$current_currency) }}-->
                                        {{ currencyPosition($earning->amount) }}
                                        </td>
                                        <td class="text-muted">
        ...
        <div class="col-12 col-lg-8">
            <div class="card">
                <div class="card-header">
                    <div class=" d-flex justify-content-between align-items-center">
                        <h3 class="card-title">{{ __('recently_purchased_orders') }} Resdex</h3>
                        <a href="{{ route('order.index_resdex') }}" class="btn btn-dark">{{ __('view_all') }}</a>
                    </div>
                </div>
                <div class="card-body table-responsive p-0">
                    <table class="table">
                        ...
                        <tbody>
                            @if ($latest_earnings_resdex->count() > 0)
                                @foreach ($latest_earnings_resdex as $earning_resdex)
                                    <tr>
                                        <td>
                                            #{{ $earning_resdex->order_id }}
                                        </td>
                                        <td class="text-muted">
                                        <!--{{ currencyPosition(currencyConversion($earning_resdex->usd_amount, 'USD', config('templatecookie.currency', 'USD') ), false,$current_currency) }}-->
                                        {{ currencyPosition($earning_resdex->amount) }}
                                        </td>
                                        <td class="text-muted">
                                            @if ($earning_resdex->payment_type == 'per_job_based')
                                                <span class="badge badge-secondary">{{ ucfirst(Str::replace('_', ' ', $earning_resdex->payment_type)) }}</span>
                                            @else
                                                <span class="badge badge-primary">{{ $earning_resdex->plan->label }}</span>
                                            @endif
                                        </td>
                                        <td class="text-muted">
                                            @if ($earning_resdex->payment_provider == 'offline')
                                                {{ __('offline') }}
                                                @if (isset($earning_resdex->manualPayment->name))
                                                    (<b>{{ $earning_resdex->manualPayment->name }}</b>)
                                                @endif
                                            @else
                                                {{ ucfirst($earning_resdex->payment_provider) }}
                                            @endif
                                        </td>
                                        <td>
                                            @if ($earning_resdex->payment_status == 'paid')
                                                <span class="badge badge-pill bg-success">{{ __('paid') }}</span>
                                            @else
                                                <span class="badge badge-pill bg-warning">{{ __('unpaid') }}</span>
                                            @endif
                                        ...
                                        <td class="text-muted">
                                            {{ $earning_resdex->created_at->diffForHumans() }}
                                        </td>
                                        <td class="text-muted">
                                        <td class="d-flex">
                                            <a href="{{ route('order.show_resdex', $earning_resdex->id) }}"
                                                class="btn bg-primary mr-1">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <form action="{{ route('admin.transaction.invoice.download_resdex', $earning_resdex->id) }}"
                                                method="POST">
                                                @csrf
                                                <button type="submit" class="btn btn-info">
                                                    <i class=" fas fa-download"></i>
