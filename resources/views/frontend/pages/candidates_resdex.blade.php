...
@section('main')
    <!-- Filter Component -->
    <x-website.candidate.candidate-filter-resdex :popular-tags="$popularTags" :professions="$professions" :experiences="$experiences" :educations="$educations" :skills="$skills" />

<!--<a href="{{ route('website.candidate.bulk_download_resume') }}" class="btn btn-primary">Download Bulk Resumes</a>-->
    <!--  canidates   -->
                ...
                @if ($candidates->count() > 0)
                    <div class="row">
                        @foreach ($candidates as $candidate)
                            <div class="@if (request('education') || request('gender') || request('experience') || request('skills')) col-md-6 fade-in-bottom  condition_class rt-mb-24 @else col-md-6 fade-in-bottom  condition_class rt-mb-24 col-xl-6 @endif">
                                <a onclick="showCandidateProfileModalResdex('{{ $candidate->user->username }}')"
                                        ...
                                        <div class="rt-single-icon-box icb-clmn-lg tw-reltaive">
                                            ...
                                            <div class="iconbox-content !tw-m-0">
                                                <div class="job-mini-title">
                                                    @if (auth('user')->check())
                                                        <span>
                                                            <small>Name:{{ $candidate->user->name }}<br>
                                                            Email:{{ $candidate->user->email }} </small>
                                                        </span>
                                                    @else
                                                ...
                                                <div class="bottom-link mt-1">
                                                    @if (auth('user')->check())
                                                        <span class="body-font-4 text-primary-500">View Profile
                                                            <x-svg.arrow-right-icon />
                                                        </span>
                                                    @else
                                                        <span class="body-font-4 text-primary-500 login_required">View Profile
                                                            <x-svg.arrow-right-icon />
                                                        </span>
                                                    @endif
                                                </div>
                                            ...
                                            <div class="tw-inline-flex tw-justify-center tw-items-center tw-absolute tw-top-3 tw-right-3 tw-text-[#767F8C]">
                                                ...
                                                @else
                                                    <div data-bs-toggle="tooltip" data-bs-placement="top" class="cursor-pointer ml--10px">
                                        ...
                                        <div class="iconbox-content">
                                            <div class="post-info2">
                                                <div class="post-main-title">
                                                    <a target="_blank"
                                                        href="{{ route('company.bookmark.category.index_resdex') }}">
...
@section('script')
    ...
    var candId;
        ...
        function showCandidateProfileModalResdex(username) {
            console.log('profile click');
            $.ajax({
                url: "{{ route('website.candidate.profile.details_resdex') }}",
                type: "GET",
                data: {
                    username: username,
                    count_view: 1
...
// 
                    ...
                    GetCandidateId(data.candidate.id);
                    console.log("check candidate id");
                    console.log(data.candidate.id);
                    // if (data.candidate.bookmarked) {
                    if (data.candidate.bookmarked && data.candidate.bookmarked === 1) {
                    ...
                    $('#candidate-profile-modal').modal('show');    // model opening here
    ...
    // Second AJAX call test
    //   $.ajax({
    //     url: '/candidates/download/cv/1',
    //     method: 'GET',
    //     headers: {
    ...
    //       console.error('Error in endpoint 2:', error2);
    //     }
    //   });
    
    
// woring fine for resume download
if(response.resume_id && response.resume_id !=0){
    console.log('resume downloading....');
    var getid=response.resume_id;
    // var getid=3;
    $.ajax({
    // url: '/candidates/download/cv/1',
    url: '/candidates/download/cv/' + getid,
    method: 'GET',
    headers: {
        'Accept': 'application/json, text/plain, */*'
    },
    xhrFields: {
        responseType: 'blob' // Set the responseType to 'blob' to handle binary responses
    },
    success: function(data) {
        ...
        var filename = 'resume_' + timestamp + '.pdf';
        a.download = filename;
    ...
    error: function (error2) {
        ...
        function CompanyBookmark(candidate) {
            $.ajax({
                url: "{{ route('company.bookmark.category.index_resdex', ['ajax' => 1]) }}",
                type: "GET",
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        function BookmarkCanidate(id, cat) {
            var url = "{{ route('company.companybookmarkcandidate_resdex', ':id') }}";
...
function GetCandidateId(candidateId) {
  ...
  console.log(candidateId);
        ...
        $('#bookmakCandidate').on('click', function() {
            console.log("sample candidate id");
            console.log(candId);
            CompanyBookmark(candId);
