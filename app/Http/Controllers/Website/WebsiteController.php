...
use App\Services\Website\PricePlanResdexService;
use App\Services\Website\Candidate\CandidateProfileDetailsServiceResdex;
...
class WebsiteController extends Controller
{
    ...
    public function abroad_jobs(Request $request)
    {
        try {
            $request->is_abroad = true;
            $data = (new JobListService())->jobs($request);

            // For adding currency code
            $current_currency = currentCurrency();
            //   return $data;
            return view('frontend.pages.abroad_jobs', $data, compact('current_currency'));
        } catch (\Exception $e) {
            flashError('An error occurred: ' . $e->getMessage());
    ...
    public function abroad_loadmore(Request $request)
    {
        try {
            $request->is_abroad = true;
            $data = (new JobListService())->loadMore($request);
    ...
    // added
    public function bulkDownloadResume()
    {

        // https://fobes.mindmadetech.in/uploads/file/candidates/nPStXPqcHEIGk9zg56R5GNWkSXxFbJ1mp1OFInuk.pdf

        // Get file paths from the database
        $filePaths = CandidateResume::pluck('file')->toArray();
        // return $filePaths;
        // Create a zip file
        $zipFileName = 'resumes.zip';
        $zip = new \ZipArchive;
        ...
        // $destinationDirectory = public_path('candidates_bulk_resume');
        // $destinationPath = $destinationDirectory . '/' . $zipFileName;
        // File::move($zipFileName, $destinationPath);
        // // return redirect(url('/candidates/' . $zipFileName));  // Redirect to the URL
        // return redirect(url('/candidates/'));  // Redirect to the URL
    }
    ...
    public function candidateProfileDetails(Request $request)
    {
        try {
            ...
            $expiryDate = Carbon::parse($user_plan->plan_expired_at);
            // Compare the expiry date with the current date and time
            if (isset($user_plan) && $expiryDate < Carbon::now()) {
                return response()->json([
                    'message' => __('Your plan has been expired'),
                    'success' => false,
    ...
    public function delete_user($id)
    {
        try {
            //print_r($id);die;
            return view('frontend.pages.delete_user', compact('id'));
        } catch (\Exception $e) {
            flashError('An error occurred: ' . $e->getMessage());
    ...
    public function pricing()
    {
        try {
            ...
            // return $plans;
            return view('frontend.pages.pricing', compact('plans', 'current_language', 'plan_descriptions', 'current_currency', 'current_language_code', 'resdex_plans'));
        } catch (\Exception $e) {
            flashError('An error occurred: ' . $e->getMessage());
            return back();
        }
    ...
    public function planResdexDetails($label)
    {
        try {
            abort_if(!auth('user')->check(), 404);
            abort_if(auth('user')->check() && auth('user')->user()->role == 'candidate', 404);

            $data = (new PricePlanResdexService())->details($label);
            // return 2;
            return view('frontend.pages.plan-resdex-details', $data);
        } catch (\Exception $e) {
            flashError('An error occurred: ' . $e->getMessage());
    ...
    public function candidateProfileDetailsResdex(Request $request)
    {
        try {
            if (!auth('user')->check()) {
                return response()->json([
            ...
            if ($user->role != 'company') {
                ...
                $user_plan = $user->company->userPlanResdex;
            }
            if (!$user_plan) {
                return response()->json([
                    'message' => __('you_dont_have_a_chosen_plan_please_choose_a_plan_to_continue'),
            ...
            if (isset($user_plan) && $user_plan->credit_limit <= 0) {
                return response()->json([
                    'message' => __('Your credit limit has been over'),
                    'success' => false,
                    'redirect_url' => route('website.plan'),
                ]);
            }

            $data = (new CandidateProfileDetailsServiceResdex())->execute($request);
