...
$result = $newDate->format('Y-m-d H:i:s');
    ...
    public function paymentPlaceResdex(Request $request)
    {
        $job_payment_type = session('job_payment_type') ?? 'package_job';
        ...
        if ($job_payment_type == 'per_job') {
            ...
            $plan = PlanResdex::findOrFail($request->plan_id);
            $price = $plan->price;
        }
        ...
        return redirect()->route('company.resdex_plans');
    }

    public function markPaidResdex(EarningResdex $order)
    {
        // payment mark as paid
        $order->update(['payment_status' => 'paid']);

        // update user plan
        $plan = PlanResdex::findOrFail($order->plan_id);
        $user_plan = UserPlanResdex::where('company_id', $order->company_id)->first();
        $company = Company::where('id', $order->company_id)->first();
