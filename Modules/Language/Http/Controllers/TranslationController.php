...
class TranslationController extends Controller
{
    ...
    public function langView($code)
    {
        try {
            ...
            // Set the number of items per page to 900
            $perPage = 100;

            // Slice the translations based on the current page
            $currentPage = request()->input('page', 1);
            $currentPageTranslations = array_slice($translations, ($currentPage - 1) * $perPage, $perPage);

            // Create a LengthAwarePaginator instance
            $translations = new LengthAwarePaginator(new Collection($currentPageTranslations), count($translations), $perPage, $currentPage, ['path' => route('languages.view', ['code' => $code])]);
