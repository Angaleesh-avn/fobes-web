...
it('redirects to the login page after clicking the login button', function () {
    ...
    $this->post(route('login'), [
        'email' => $user->email,
        'password' => 'password',
    ])->assertRedirect(route('website.candidate'));
});
...
it('displays canidate resume modal ', function () {
    $this->seed([
...
it('displays canidate resume kanbanboard modal', function () {
...
it('displays apply job modal', function () {
    $this->seed([
        IndustryTypeSeeder::class, OrganizationTypeSeeder::class,  TeamSizeSeeder::class, ProfessionSeeder::class, EducationSeeder::class, ExperienceSeeder::class, CompanySeeder::class, JobCategorySeeder::class, JobTypeSeeder::class, SalaryTypeSeeder::class, JobRoleSeeder::class, JobSeeder::class,
    ]);
    $user = User::factory()->create(['role' => 'candidate']);
    $job = Job::find(35);
